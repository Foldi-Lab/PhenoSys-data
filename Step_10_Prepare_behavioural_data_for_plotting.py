...
# Row labels.csv
# Red 0 Session 1 (2022-02-18) (576x432)_RMD.csv
# Red 0 Session 1 (2022-02-18) (576x432)_RMD_RFN_boutlengths.csv
# Red 0 Session 1 (2022-02-18) (576x432)_RMD_RFN_behavsperframe.csv
...
def find_time_spent_doing_behaviours(import_location):
    
    print('Find the total time spent doing each behaviour.')
    
    import_paths = os.path.join(import_location, '*_boutlengths.csv')
    ...
    shortened_row_info = row_info.loc[master.index]
    master_export = pd.concat([shortened_row_info, master], axis=1)
    
    # Export the master dataframe.
    export_name = 'Video time results file.csv'
    export_destination = os.path.join(import_location, export_name)
    master_export.to_csv(export_destination, index=False)

def add_missing_rodent_data_back(import_location):
    
    print('Add missing rodent data back, to create time bin data.')
    
    import_paths = os.path.join(import_location, '*_behavsperframe.csv')
    import_paths = glob(import_paths)
    zone_paths   = os.path.join(import_location, '*_RMD.csv')
    zone_paths   = glob(zone_paths)
    
    for path in tqdm(import_paths, ncols=70):
        
        # Create a list of behaviours with the corresponding frame numbers.
        df_behav = pd.read_csv(path, skiprows=[1,2], usecols=[1])
        zones_file_path = path[:-27] + '_RMD.csv'
        ...
        # Export the results.
        export_path = path[:-4] + '_AMD.csv'
        df_results.to_csv(export_path, index=False)

def find_time_spent_doing_behaviours_per_time_bin(import_location):
    
    print('Find the time spent doing each behaviour per time bin.')
    
    import_paths = os.path.join(import_location, '*_behavsperframe_AMD.csv')
    import_paths = glob(import_paths)
    row_info_path = os.path.join(import_location, 'Row labels.csv')
    ...
    for stat in stats_to_use:
        ...
        # Import the row info and make the columns a multiindex of the column
        # headings and a row of 'Blank' values.
        row_info  = pd.read_csv(row_info_path, header=None)
        header    = pd.DataFrame(row_info.iloc[0]).T
        header.loc[1] = ['Blank']*len(header.iloc[0])
        row_info = row_info.loc[1:]
        row_info.columns = pd.MultiIndex.from_frame(header.T)
        
        # Append this row info to the time bin data.
        row_info.index = row_info[('Full name','Blank')]
        shortened_row_info = row_info.loc[master[stat].index]
        master_export[stat] = pd.concat([shortened_row_info, master[stat]], axis=1)
        
        # Export the master dataframes.
        export_name = ('Time bins results '+'('+stat+') file.csv').replace('/','(div)')
        export_destination = os.path.join(import_location, export_name)
        master_export[stat].to_csv(export_destination, index=False)

if __name__ == "__main__":
    
    find_time_spent_doing_behaviours(import_location)
    add_missing_rodent_data_back(import_location)
    find_time_spent_doing_behaviours_per_time_bin(import_location)
